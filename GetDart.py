...
def drawBoard():
    ...
    # draw board
    cv2.circle(raw_loc_mat, (400, 400), 170 * 2,
               (255, 255, 255), 1)  # outside double
    cv2.circle(raw_loc_mat, (400, 400), 160 * 2,
               (255, 255, 255), 1)  # inside double
    cv2.circle(raw_loc_mat, (400, 400), 107 * 2,
               (255, 255, 255), 1)  # outside treble
    cv2.circle(raw_loc_mat, (400, 400), 97 * 2,
               (255, 255, 255), 1)  # inside treble
...
def DartLocation(x_coord, y_coord):
    try:
        ...
        dart_loc = cv2.perspectiveTransform(
            dart_loc_temp, transformation_matrix)
...
def DartRegion(dart_loc):
    try:
        ...
        dart_angle = math.fmod(
            ((math.atan2(vy, vx) * 180/math.pi) + 360 - ref_angle), 360)
    ...
    while success:
        ...
        if cv2.countNonZero(thresh) > x and cv2.countNonZero(thresh) < 15000:
            ...
            edges = cv2.goodFeaturesToTrack(
                blur, 640, 0.0008, 3, mask=None, blockSize=3, useHarrisDetector=1, k=0.06)  # k=0.08
            ...
            [vx, vy, x, y] = cv2.fitLine(
                corners_new, cv2.DIST_HUBER, 0, 0.1, 0.1)
            lefty = int((-x*vy/vx) + y)
            ...
            # check max pos!!!, write image with circle??!!!
            ...
            try:
                ...
                cv2.circle(t_plus_copy, (locationofdart.item(
                    0), locationofdart.item(1)), 10, (0, 0, 0), 2, 8)
                cv2.circle(t_plus_copy, (locationofdart.item(
                    0), locationofdart.item(1)), 2, (0, 0, 0), 2, 8)

                # check for the location of the dart with the calibration

                dartloc = DartLocation(
            ...
            if DEBUG:
                # 400 + dartInfo.magnitude * math.tan(dartInfo.angle * math.pi/180)
                loc_x = dartloc[0]
                # 400 + dartInfo.magnitude * math.tan(dartInfo.angle * math.pi/180)
                loc_y = dartloc[1]
                cv2.circle(debug_img, (int(loc_x), int(loc_y)),
                           2, (0, 255, 0), 2, 8)
                cv2.circle(debug_img, (int(loc_x), int(loc_y)),
                           6, (0, 255, 0), 1, 8)
                string = "" + str(dartInfo.base) + "x" + \
                    str(dartInfo.multiplier)
                # add text (before clear with rectangle)
                cv2.rectangle(debug_img, (600, 700), (800, 800), (0, 0, 0), -1)
                cv2.putText(debug_img, string, (600, 750),
                            cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 2, 8)
